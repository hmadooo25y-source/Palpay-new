...
async function loadSettingsFromDB() {
    try {
        ...
        // تطبيق بيانات التاجر
        if (record.merchant_name)  localStorage.setItem('merchant_name',  record.merchant_name);
        if (record.merchant_phone) {
            localStorage.setItem('merchant_phone', record.merchant_phone);
            const s8Input = document.getElementById('merchant-phone-input');
            if (s8Input) s8Input.value = record.merchant_phone;
        }
