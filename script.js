...
async function loadSettingsFromDB() {
    try {
        ...
        // تطبيق أول تاجر كافتراضي (للتوافق مع QR)
        const merchants = record.merchants || [];
        if (merchants.length > 0) {
            localStorage.setItem('merchant_name',  merchants[0].name);
            localStorage.setItem('merchant_phone', merchants[0].phone);
        }

        // عرض قائمة التجار في screen-8
        renderMerchantsInS8(merchants);
